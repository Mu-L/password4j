...
public class Blake2bTest
{
    ...
    public void test()
    {
        for (TestCase test : CASES)
        {
            ...
            instance.update(test.message == null ? null : test.message.getBytes(P4jUtils.DEFAULT_CHARSET));
            byte[] out = new byte[test.length];
            instance.doFinal(out, 0);
            assertEquals(test.expected, P4jUtils.toHex(out));
        }
    }
    ...
    public void parallelTest() throws InterruptedException, ExecutionException
    {
        ...
        for (final TestCase test : CASES)
        {
            Callable<Boolean> c = () -> {
                ...
                byte[] out = new byte[test.length];
                instance.doFinal(out, 0);
                return test.expected.equals(P4jUtils.toHex(out));
            };
            tasks.add(c);
        }

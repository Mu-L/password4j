...
public class CompressedPBKDF2Function extends PBKDF2Function
{
    ...
    private static final char DELIMITER = P4jPropertyReader.readChar("hash.pbkdf2.delimiter", '$');
    ...
    protected String getHash(byte[] encodedKey, String salt)
    {
        ...
        String salt64 = P4jUtils.encodeBase64(P4jUtils.fromCharSequenceToBytes(salt));
    ...
    private String getSaltFromHash(String hashed)
    {
        ...
        if (parts.length == 5)
        {
            return P4jUtils.fromBytesToString(P4jUtils.decodeBase64(P4jUtils.fromCharSequenceToBytes(parts[3])));
        }
        throw new BadParametersException("`" + hashed + "` is not a valid hash");
    }
}

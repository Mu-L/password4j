...
public class ScryptFunction extends AbstractHashingFunction
{
    ...
    public static ScryptFunction getInstanceFromHash(String hashed)
    {
        ...
        if (parts.length == 5)
        {
            ...
            int derivedKeyLength = P4jUtils.decodeBase64(parts[4]).length;
    ...
    {
        byte[] saltAsBytes = P4jUtils.fromCharSequenceToBytes(salt);
        return internalHash(plainTextPassword, saltAsBytes);
    }

    private Hash internalHash(CharSequence plainTextPassword, byte[] salt)
    {
        String stringedSalt = P4jUtils.fromBytesToString(salt);
        try
        {
            byte[] derived = scrypt(P4jUtils.fromCharSequenceToBytes(plainTextPassword), salt, derivedKeyLength);
            String params = Long.toString((long) P4jUtils.log2(workFactor) << 16 | (long) resources << 8 | parallelization, 16);
            String sb = "$s0$" + params + '$' + P4jUtils.encodeBase64(salt) + '$'
                    + P4jUtils.encodeBase64(derived);
    ...
    public boolean check(CharSequence plainTextPassword, String hashed)
    {
        try
        {
            ...
            {
                byte[] salt = P4jUtils.decodeBase64(parts[3]);
                byte[] derived0 = P4jUtils.decodeBase64(parts[4]);
                byte[] derived1 = scrypt(P4jUtils.fromCharSequenceToBytes(plainTextPassword), salt, derivedKeyLength);
    ...
    public byte[] scrypt(byte[] passwd, byte[] salt, int dkLen) throws GeneralSecurityException
    {
        if (workFactor >= 2 && (workFactor & workFactor - 1) == 0)
        {
            ...
            else
            {
                ...
                byte[] intensiveSalt = PBKDF2Function.internalHash(P4jUtils.fromBytesToString(passwd).toCharArray(), salt, Hmac.SHA256.name(), 1,
                ...
                return PBKDF2Function
                        .internalHash(P4jUtils.fromBytesToString(passwd).toCharArray(), intensiveSalt, Hmac.SHA256.name(), 1, 8 * dkLen)
                        .getEncoded();
            }
        }
        else

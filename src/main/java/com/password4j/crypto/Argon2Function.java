...
public class Argon2Function extends AbstractHashingFunction
{
    ...
    private static void updateWithLength(Blake2b blake2b, byte[] input)
    {
        if (input != null)
        {
            blake2b.update(P4jUtils.intToLittleEndianBytes(input.length));
            blake2b.update(input);
        }
        else
        {
            blake2b.update(P4jUtils.intToLittleEndianBytes(0));
        }
    }
    ...
    private static void fillBlock(long[] x, long[] y, long[] currentBlock, boolean withXor)
    {
        ...
        if (withXor)
        {
            P4jUtils.xor(currentBlock, r, z, currentBlock);
        }
        else
        {
            P4jUtils.xor(currentBlock, r, z);
        }
    }
    ...
    private static Object[] decodeHash(String hash)
    {
        ...
        if (parts.length == 6)
        {
            ...
            result[5] = P4jUtils.decodeBase64(parts[4]);
            result[6] = P4jUtils.decodeBase64(parts[5]);
            return result;
        }
        else
        {
    ...
    {
        return internalHash(plainTextPassword, P4jUtils.fromCharSequenceToBytes(salt), pepper);
    }

    private Hash internalHash(CharSequence plainTextPassword, byte[] salt, CharSequence pepper)
    {
        byte[] password = P4jUtils.fromCharSequenceToBytes(plainTextPassword);
        ...
        String theSalt = P4jUtils.fromBytesToString(salt);
        initialize(password, salt, P4jUtils.fromCharSequenceToBytes(pepper), null, blockMemory);
    ...
    {
        Object[] params = decodeHash(hashed);
        return check(plainTextPassword, hashed, P4jUtils.fromBytesToString((byte[]) params[5]), null);
    }
    ...
    public boolean check(CharSequence plainTextPassword, String hashed, String salt, CharSequence pepper)
    {
        ...
        {
            Object[] params = decodeHash(hashed);
            theSalt = P4jUtils.fromBytesToString((byte[]) params[5]);
        }
        else
        {
            theSalt = salt;
    ...
    private void initialize(byte[] plainTextPassword, byte[] salt, byte[] secret, byte[] additional, long[][] blockMemory)
    {
        ...
        blake2b.update(P4jUtils.intToLittleEndianBytes(parallelism));
        blake2b.update(P4jUtils.intToLittleEndianBytes(outputLength));
        blake2b.update(P4jUtils.intToLittleEndianBytes(memory));
        blake2b.update(P4jUtils.intToLittleEndianBytes(iterations));
        blake2b.update(P4jUtils.intToLittleEndianBytes(version));
        blake2b.update(P4jUtils.intToLittleEndianBytes(variant.ordinal()));
        ...
        for (int i = 0; i < parallelism; i++)
        {
            ...
            byte[] iBytes = P4jUtils.intToLittleEndianBytes(i);
            ...
            blockMemory[i * laneLength] = P4jUtils.fromBytesToLongs(blockHashBytes);

            blockHashBytes = blake2bLong(initialHashWithOnes, ARGON2_BLOCK_SIZE);
            blockMemory[i * laneLength + 1] = P4jUtils.fromBytesToLongs(blockHashBytes);
        }
    ...
    private byte[] blake2bLong(byte[] input, int outputLength)
    {
        ...
        byte[] outlenBytes = P4jUtils.intToLittleEndianBytes(outputLength);
    ...
    private void initAddressBlocks(int pass, int lane, int slice, long[] zeroBlock, long[] inputBlock, long[] addressBlock,
            long[][] blockMemory)
    {
        inputBlock[0] = P4jUtils.intToLong(pass);
        inputBlock[1] = P4jUtils.intToLong(lane);
        inputBlock[2] = P4jUtils.intToLong(slice);
        inputBlock[3] = P4jUtils.intToLong(blockMemory.length);
        inputBlock[4] = P4jUtils.intToLong(iterations);
        inputBlock[5] = P4jUtils.intToLong(variant.ordinal());
    ...
    private byte[] ending(long[][] blockMemory)
    {
        ...
        for (int i = 1; i < parallelism; i++)
        {
            ...
            P4jUtils.xor(finalBlock, blockMemory[lastBlockInLane]);
        }
        ...
        {
            byte[] bytes = P4jUtils.longToLittleEndianBytes(finalBlock[i]);
    ...
    private String encodeHash(byte[] hash, String salt)
    {
        return "$argon2" + variant.name()
                ...
                + P4jUtils.encodeBase64(P4jUtils.fromCharSequenceToBytes(salt), false) + "$"
                + P4jUtils.encodeBase64(hash, false);
    }
